...
public class ExpenseEditFragment extends Fragment
    ...
    @BindView(R.id.item_expense) RelativeLayout expenseEditView;
    ...
    @BindView(R.id.et_place) EditText placeView;
    ...
    @BindView(R.id.til_et_place) TextInputLayout placeContainer;
    ...
    private static final int BUDGETS_LOADER = 1;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (getArguments() != null) {
            transactionExpense = getArguments().getParcelable(ARG_TRANSACTION);
            isExpenseNew = false;
            ((MainActivity) getActivity()).getSupportActionBar().setTitle(R.string.title_edit_expense);
        } else {
            isExpenseNew = true;
        }
    }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View rootView = inflater.inflate(R.layout.fragment_item_expense, container, false);
        ...
        placeView.setOnFocusChangeListener(onFocusChangeListener);
        ...
        if (!isExpenseNew) {
            ...
            amountView.setText(getAmountStr(transactionExpense.getAmount()).replace("-",""));
            dateView.setText(getCalendarFromFormattedLong(transactionExpense.getDate().getTime()));
            datePopupWindow.setDateWheel(transactionExpense.getDate().getTime());
            placeView.setText(transactionExpense.getPlace());
            notesView.setText(transactionExpense.getNotes());
        } else {
            ...
            switch (v.getId()) {
                case (R.id.et_amount):
                    if (hasFocus) {
                        ...
                        calculatorPopupWindow.showAtLocation(expenseEditView, Gravity.BOTTOM, 0, 0);
                ...
                case (R.id.et_account):
                    if (hasFocus) {
                        if (accountsPopupWindow.getCurrentParentID() == -1) {
                            ...
                            accountsPopupWindow.showAtLocation(expenseEditView, Gravity.BOTTOM, 0, 0);
                        }
                    }else {
                ...
                case (R.id.et_category):
                    if (hasFocus) {
                        if (budgetsPopupWindow.getCurrentParentID() == -1) {
                            ...
                            budgetsPopupWindow.showAtLocation(expenseEditView, Gravity.BOTTOM, 0, 0);
                        }
                    } else {
                ...
                case (R.id.et_date):
                    if (hasFocus) {
                        ...
                        datePopupWindow.showAtLocation(expenseEditView, Gravity.BOTTOM, 0, 0);
                    } else {
                ...
                case (R.id.et_place):
                    if (hasFocus) {
                        placeContainer.setBackground(backgroundFocus);
                    } else {
                        placeContainer.setBackground(backgroundWithoutFocus );
                        hideKeyboard(getActivity(),v);
                    }
                    break;
                case (R.id.et_notes):
                    if (hasFocus) {
                        notesContainer.setBackground(backgroundFocus);
                    } else {
                        notesContainer.setBackground(backgroundWithoutFocus);
                        hideKeyboard(getActivity(),v);
                    }
                    break;
            }
    ...
    public void deleteTransaction() {
        getActivity().getContentResolver().delete(TransactionEntry.CONTENT_URI,
                ...
                new String[]{String.valueOf(transactionExpense.getId())});
    ...
    private ContentValues getExpenseContentValues(){
        ContentValues contentValues = new ContentValues();
        contentValues.put(TransactionEntry.COLUMN_TRANSACTION_TYPE, "0");
        BigDecimal expenseAmount = new BigDecimal("-"+getEditAmountText());
        contentValues.put(TransactionEntry.COLUMN_TRANSACTION_AMOUNT,
                bigDecimalToDbVal(expenseAmount,accountsPopupWindow.getCurrencyUnit()));
        contentValues.put(TransactionEntry.COLUMN_TRANSACTION_CURRENCY, accountsPopupWindow.getCurrentAccountCurrency());
        ...
        contentValues.put(TransactionEntry.COLUMN_TRANSACTION_PLACE,
                String.valueOf(placeView.getText()));
        contentValues.put(TransactionEntry.COLUMN_TRANSACTION_NOTES,
                String.valueOf(notesView.getText()));
        contentValues.put(TransactionEntry.COLUMN_TRANSACTION_PHOTO_DIR, "");
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
        switch (loader.getId()) {
            case ACCOUNTS_LOADER:
                ...
                if (!isExpenseNew) {
                    accountsPopupWindow.initWheelPosition(transactionExpense.getMainAccount());
                    accountView.setText(accountsPopupWindow.getDisplayNameStr());
                } else {
                    accountView.setText(accountsPopupWindow.getDisplayNameStr());
                }
            ...
            case BUDGETS_LOADER:
                ...
                if (!isExpenseNew)
                    budgetsPopupWindow.initWheelPosition(transactionExpense.getMainBudgetID(), transactionExpense.getSubBudgetID());
